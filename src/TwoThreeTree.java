...
class Node<T extends Comparable<T>> {
    ...
    public static <T extends Comparable<T>> Node<T> newTwoNode(T value) {
    ...
    public static <T extends Comparable<T>> Node<T> newThreeNode(T leftVal, T rightVal) {
        Node<T> node = new Node<T>();
 ...
 */
final class HoleNode<T extends Comparable<T>> extends Node {
...
@SuppressWarnings("unchecked")
public class TwoThreeTree<T extends Comparable<T>> extends AbstractSet<T> implements SortedSet<T>, iArboles<T> {
    ...
    public T find(T value){
    	...
    	if(node != null){
    		...
    		if(value.compareTo(node.val())==0){
    			return node.val();
    		}else if(value.compareTo(node.leftVal())==0){
    			return node.leftVal();
    		}else{
